...
def test_png_written_and_linked(tmp_path):
    ...
    result, written, warnings = externalize_figures(md, output, Path("assets"))

    assert warnings == []
    assert written == [tmp_path / "assets" / "figure-1.png"]
    assert written[0].read_bytes() == _PNG_BYTES
    assert "![png](assets/figure-1.png)" in result
...
def test_raw_svg_written_as_svg_file(tmp_path):
    ...
    assert warnings == []
    assert written == [tmp_path / "assets" / "figure-1.svg"]
    assert written[0].read_text() == _SVG_MARKUP
    assert "![svg](assets/figure-1.svg)" in result
...
def test_relative_dir_resolved_against_output_parent(tmp_path):
    ...
    assert written == [tmp_path / "docs" / "assets" / "img" / "figure-1.png"]
    assert written[0].is_file()
    assert "![png](assets/img/figure-1.png)" in result


def test_absolute_dir_gives_absolute_link(tmp_path):
    output = tmp_path / "example.md"
    assets_dir = tmp_path / "elsewhere" / "assets"
    md = f"{_img(_PNG_DATA)}\n"

    result, written, _ = externalize_figures(md, output, assets_dir)

    assert written == [assets_dir / "figure-1.png"]
    assert f"![png]({assets_dir.as_posix()}/figure-1.png)" in result
...
def test_nested_img_keeps_tag_and_repoints_src(tmp_path):
    ...
    result, written, _ = externalize_figures(md, output, Path("assets"))

    assert len(written) == 1
    assert '<img src="assets/figure-1.png" alt="png">' in result
...
def test_multiple_figures_numbered_in_document_order(tmp_path):
    ...
    assert result.index("figure-1.png") < result.index("figure-2.svg")
    assert result.index("figure-2.svg") < result.index("figure-3.png")
...
def test_unknown_image_mime_falls_back_to_subtype(tmp_path):
    ...
    _, written, warnings = externalize_figures(md, output, Path("assets"))

    assert warnings == []
    assert written == [tmp_path / "assets" / "figure-1.xfake"]
...
def test_indented_img_still_treated_as_own_line(tmp_path):
    ...
    result, _, _ = externalize_figures(md, output, Path("assets"))
